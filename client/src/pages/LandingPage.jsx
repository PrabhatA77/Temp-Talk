...
const LandingPage = () => {
    ...
    const handleOpenAuth = () => {
        ...
        //add a tiny delay to give the bar time to move ,then open modal
        setTimeout(() => {
            setIsModalOpen(true);
            nprogress.done(); //stop the bar
        }, 400)
    };
 ...
 min-h-screen text-white'>
            <nav className='bg-[#0d1412] flex justify-between items-center px-8 h-16 border-b border-gray-800'>
                ...
                <div className='flex items-center gap-4 text-sm'>
                    <div onClick={handleOpenAuth} className='hover:text-emerald-400 transition cursor-pointer'>Login</div>
                    <div onClick={handleOpenAuth} className='bg-emerald-500 px-5 py-2 rounded-md hover:bg-emerald-600 transition shadow shadow-emerald-500/30 cursor-pointer'>Get Started</div>
                </div>
            </nav>
      ...
      ease-out'>
            ...
            {isModalOpen && (
                <div className='fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-md'>
                    {/* the actual signup foem floating in the center */}
                    <div className='relative w-full max-w-md'>
                        {/* close button for the modal */}
                        <button
                            onClick={()=>setIsModalOpen(false)}
                            className='absolute top-4 right-4 z-50 text-gray-500 hover:text-white transition'
                        >
                            ✕
                        </button>
                        {/* passing a prop to signup page so it knows it is inside a modal */}
